...
U8 boot_loader_flash_data_au8[1024];
U8 read_boot_loader_buffer_au8[1024];
...
static void bootloader_enable_cmd_procees(void);
static void bootloader_new_firmware_receive(void);
static void bootloader_whole_crc_check(void);
static void bootloader_end_of_data_handling(void);
...
bootloader_app_sts_te boot_loader_app_check_new_firmware_e(void)
{
    ...
    if(internal_flash_app_read_e(BOOTLOADER_CONFIG_START_ADDRESS, (U8*)&boot_loader_conf_gst, sizeof(boot_loader_conf_gst)) == IN_FLASH_SUCCESS)
    {
        ...
        else if(0xCC == boot_loader_conf_gst.new_firmware_status_u8)
        {
        	boot_loader_status_e =  BOOT_LOADER_RECEIVE_NEW_FIRMWARE;
